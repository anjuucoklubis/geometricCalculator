...
export default function MyKeyboardCalPangkat() {
    ...
    <View style={Styles.viewButton}>
      <View
        ...
        <Text style={{fontSize: 35, color: myColors.gray, fontWeight: '200'}}>
          Hitung Pangkat {inputNumber}
        </Text>
        {result !== null ? (
          <Text style={{fontSize: 35, color: myColors.gray, fontWeight: '200'}}>
            adalah  <Text style={{color: myColors.result}}>{result.toString()}</Text>
          </Text>
        ) : (
          <Text>{''}</Text>
        )}
      </View>
      <TouchableOpacity onPress={calculateSquare}>
        <View
          style={{
            maxWidth: '100%',
            flexDirection: 'row',
